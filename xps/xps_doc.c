...
xps_add_link_target(xps_context *ctx, char *name)
{
	...
	xps_target *target = fz_malloc(ctx->ctx, sizeof *target);
	target->name = fz_strdup(ctx->ctx, name);
...
xps_free_link_targets(xps_context *ctx)
{
	...
	{
		next = target->next;
		fz_free(ctx->ctx, target->name);
		fz_free(ctx->ctx, target);
		target = next;
	}
}
...
xps_free_fixed_documents(xps_context *ctx)
{
	...
	while (fixdoc)
	{
		...
		fz_free(ctx->ctx, fixdoc->name);
		fz_free(ctx->ctx, fixdoc->outline); /* SumatraPDF: fix memory leak */
		fz_free(ctx->ctx, fixdoc);
		fixdoc = next;
	}
		...
		if (!strcmp(xml_tag(item), "Relationship"))
		{
			...
			if (target && type)
			{
				...
				if (!strcmp(type, REL_DOC_STRUCTURE) && fixdoc)
					fixdoc->outline = fz_strdup(ctx->ctx, tgtbuf);
			}
		}
...
xps_parse_metadata(xps_context *ctx, xps_part *part, xps_document *fixdoc)
{
	...
	if (!root)
		return fz_error_note(ctx->ctx, -1, "cannot parse metadata part '%s'", part->name);
...
xps_read_and_process_metadata_part(xps_context *ctx, char *name, xps_document *fixdoc)
{
	...
	if (!part)
		return fz_error_note(ctx->ctx, -1, "cannot read zip part '%s'", name);

	code = xps_parse_metadata(ctx, part, fixdoc);
	if (code)
		return fz_error_note(ctx->ctx, code, "cannot process metadata part '%s'", name);
...
xps_read_page_list(xps_context *ctx)
{
	...
	if (code)
		return fz_error_note(ctx->ctx, code, "cannot process root relationship part");

	if (!ctx->start_part)
		return fz_error_make(ctx->ctx, "cannot find fixed document sequence start part");

	code = xps_read_and_process_metadata_part(ctx, ctx->start_part, NULL);
	if (code)
		return fz_error_note(ctx->ctx, code, "cannot process FixedDocumentSequence part");
	...
	for (fixdoc = ctx->first_fixdoc; fixdoc; fixdoc = fixdoc->next)
	{
		...
		if (code)
			fz_error_handle(ctx->ctx, code, "cannot process FixedDocument rels part");

		code = xps_read_and_process_metadata_part(ctx, fixdoc->name, fixdoc);
		if (code)
			return fz_error_note(ctx->ctx, code, "cannot process FixedDocument part");
	}
...
xps_load_fixed_page(xps_context *ctx, xps_page *page)
{
	...
	if (!part)
		return fz_error_note(ctx->ctx, -1, "cannot read zip part '%s'", page->name);

	root = xml_parse_document(ctx->ctx, part->data, part->size);
	if (!root)
		return fz_error_note(ctx->ctx, -1, "cannot parse xml part '%s'", page->name);

	xps_free_part(ctx, part);

	if (strcmp(xml_tag(root), "FixedPage"))
		return fz_error_make(ctx->ctx, "expected FixedPage element (found %s)", xml_tag(root));

	width_att = xml_att(root, "Width");
	if (!width_att)
		return fz_error_make(ctx->ctx, "FixedPage missing required attribute: Width");

	height_att = xml_att(root, "Height");
	if (!height_att)
		return fz_error_make(ctx->ctx, "FixedPage missing required attribute: Height");
...
xps_load_page(xps_page **pagep, xps_context *ctx, int number)
{
	...
	for (page = ctx->first_page; page; page = page->next)
	{
		if (n == number)
		{
			if (!page->root)
			{
				...
				if (code)
					return fz_error_note(ctx->ctx, code, "cannot load page %d", number + 1);
			}
			*pagep = page;
			return fz_okay;
		}
	...
	return fz_error_make(ctx->ctx, "cannot find page %d", number + 1);
}
