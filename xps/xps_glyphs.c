...
static void
xps_select_best_font_encoding(fz_context *ctx, fz_font *font)
{
		...
		char *base_uri, xps_resource *dict, xml_element *root)
{
	...
	if (!font)
	{
		...
		if (code) {
			fz_error_handle(ctx->ctx, code, "cannot load font resource '%s'", partname);
			xps_free_part(ctx, part);
			return;
		}

		xps_select_best_font_encoding(ctx->ctx, font);
