...
static struct ft2vert_st *ft2vert_init(fz_context *ctx, FT_Face face)
{
	...
	scan_GSUB_Header(ctx, ret, ret->GSUB_table);
...
static void ft2vert_final(fz_context *ctx, FT_Face face, struct ft2vert_st *vert){
	int j;
...
static FT_UInt ft2gsub_get_gid(fz_context *ctx, const struct ft2vert_st *ft2vert, const FT_UInt gid)
{
	int i, k;
		...
		case 1:
			for (i = 0; i < t->Coverage.GlyphCount; i++)
				if (t->Coverage.GlyphArray[i] == gid)
					return get_vert_nth_gid(ctx, t, gid, i);
			break;
		case 2:
			for (i = 0; i < t->Coverage.RangeCount; i++)
			{
				...
				if (r->Start <= gid && gid <= r->End)
					return get_vert_nth_gid(ctx, t, gid, gid - r->Start + j);
				j += r->End - r->Start + 1;
			}
			break;
		default:
			fz_warn(ctx, "ft2gsub_get_gid: internal error");
		}
	}
...
int pdf_ft_get_vgid(fz_context *ctx, pdf_font_desc *fontdesc, int gid)
{
	...
	vgid = ft2gsub_get_gid(ctx, fontdesc->_vsubst, gid);
...
void pdf_ft_free_vsubst(fz_context *ctx, pdf_font_desc *fontdesc)
{
	if (fontdesc && fontdesc->_vsubst)
		ft2vert_final(ctx, fontdesc->font->ft_face, fontdesc->_vsubst);
}
