...
pdf_parse_array(fz_obj **op, pdf_xref *xref, fz_stream *file, char *buf, int cap)
{
	...
	while (1)
	{
		...
		case PDF_TOK_R:
			...
			{
				fz_drop_obj(ctx, ary);
				return fz_error_make(ctx, "cannot parse indirect reference in array");
			}
		...
		default:
			fz_drop_obj(ctx, ary);
			return fz_error_make(ctx, "cannot parse token in array");
		}
	}
}
...
skip:
		...
		{
			fz_drop_obj(ctx, dict);
			return fz_error_make(ctx, "invalid key in dict");
		}
		...
		case PDF_TOK_INT:
			...
			fz_drop_obj(ctx, key);
			fz_drop_obj(ctx, dict);
			return fz_error_make(ctx, "invalid indirect reference in dict");

		default:
			fz_drop_obj(ctx, key);
			fz_drop_obj(ctx, dict);
			return fz_error_make(ctx, "unknown token in dict");
		}
...
pdf_parse_stm_obj(fz_obj **op, pdf_xref *xref, fz_stream *file, char *buf, int cap)
{
	...
	error = pdf_lex(&tok, file, buf, cap, &len);
	if (error)
		return fz_error_note(ctx, error, "cannot parse token in object stream");
	...
	case PDF_TOK_OPEN_ARRAY:
		...
		if (error)
			return fz_error_note(ctx, error, "cannot parse object stream");
		break;
	case PDF_TOK_OPEN_DICT:
	...
	default: return fz_error_make(ctx, "unknown token in object stream");
	}
	...
	int *onum, int *ogen, int *ostmofs)
{
	...
	error = pdf_lex(&tok, file, buf, cap, &len);
	if (error)
		return fz_error_note(ctx, error, "cannot parse indirect object (%d %d R)", num, gen);
	if (tok != PDF_TOK_INT)
		return fz_error_make(ctx, "expected object number (%d %d R)", num, gen);
	num = atoi(buf);
	...
	if (error)
		return fz_error_note(ctx, error, "cannot parse indirect object (%d %d R)", num, gen);
	if (tok != PDF_TOK_INT)
		return fz_error_make(ctx, "expected generation number (%d %d R)", num, gen);
	gen = atoi(buf);
	...
	if (error)
		return fz_error_note(ctx, error, "cannot parse indirect object (%d %d R)", num, gen);
	if (tok != PDF_TOK_OBJ)
		return fz_error_make(ctx, "expected 'obj' keyword (%d %d R)", num, gen);
	...
	case PDF_TOK_INT:
		...
		return fz_error_make(ctx, "expected 'R' keyword (%d %d R)", num, gen);
	...
	default:
		return fz_error_make(ctx, "syntax error in object (%d %d R)", num, gen);
	}
