...
fz_init_freetype(fz_context *ctx)
{
	...
	fterr = FT_Init_FreeType(&ctx->ft->ftlib);
	if (fterr)
		return fz_error_make(ctx, "cannot init freetype: %s", ft_error_string(fterr));

	FT_Library_Version(ctx->ft->ftlib, &maj, &min, &pat);
	if (maj == 2 && min == 1 && pat < 7)
	{
		fterr = FT_Done_FreeType(ctx->ft->ftlib);
		if (fterr)
			fz_warn(ctx, "freetype finalizing: %s", ft_error_string(fterr));
		return fz_error_make(ctx, "freetype version too old: %d.%d.%d", maj, min, pat);
	}
...
fz_new_font_from_file(fz_context *ctx, fz_font **fontp, char *path, int index)
{
	...
	if (error)
		return fz_error_note(ctx, error, "cannot init freetype library");
...
fz_render_ft_stroked_glyph(fz_context *ctx, fz_font *font, int gid, fz_matrix trm, fz_matrix ctm, fz_stroke_state *state)
{
	...
	fterr = FT_Stroker_New(ctx->ft->ftlib, &stroker);
	if (fterr)
	{
...
fz_render_t3_glyph(fz_context *ctx, fz_font *font, int gid, fz_matrix trm, fz_colorspace *model)
{
	...
	if (error)
		fz_error_handle(ctx, error, "cannot draw type3 glyph");
