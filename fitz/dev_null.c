...
{
	if (dev->free_user)
		dev->free_user(dev->ctx, dev->user);
	fz_free(dev->ctx, dev);
}
...
{
	if (dev->fill_path)
		dev->fill_path(dev->ctx, dev->user, path, even_odd, ctm, colorspace, color, alpha);
}
...
{
	if (dev->stroke_path)
		dev->stroke_path(dev->ctx, dev->user, path, stroke, ctm, colorspace, color, alpha);
}
...
{
	if (dev->clip_path)
		dev->clip_path(dev->ctx, dev->user, path, rect, even_odd, ctm);
}
...
fz_clip_stroke_path(fz_device *dev, fz_path *path, fz_rect *rect, fz_stroke_state *stroke, fz_matrix ctm)
{
	if (dev->clip_stroke_path)
		dev->clip_stroke_path(dev->ctx, dev->user, path, rect, stroke, ctm);
}
...
{
	if (dev->fill_text)
		dev->fill_text(dev->ctx, dev->user, text, ctm, colorspace, color, alpha);
}
...
{
	if (dev->stroke_text)
		dev->stroke_text(dev->ctx, dev->user, text, stroke, ctm, colorspace, color, alpha);
}
...
{
	if (dev->clip_text)
		dev->clip_text(dev->ctx, dev->user, text, ctm, accumulate);
}
...
fz_clip_stroke_text(fz_device *dev, fz_text *text, fz_stroke_state *stroke, fz_matrix ctm)
{
	if (dev->clip_stroke_text)
		dev->clip_stroke_text(dev->ctx, dev->user, text, stroke, ctm);
}
...
{
	if (dev->ignore_text)
		dev->ignore_text(dev->ctx, dev->user, text, ctm);
}
...
{
	if (dev->fill_shade)
		dev->fill_shade(dev->ctx, dev->user, shade, ctm, alpha);
}
...
{
	if (dev->fill_image)
		dev->fill_image(dev->ctx, dev->user, image, ctm, alpha);
}
	...
	fz_colorspace *colorspace, float *color, float alpha)
{
	if (dev->fill_image_mask)
		dev->fill_image_mask(dev->ctx, dev->user, image, ctm, colorspace, color, alpha);
}
...
fz_clip_image_mask(fz_device *dev, fz_pixmap *image, fz_rect *rect, fz_matrix ctm)
{
	if (dev->clip_image_mask)
		dev->clip_image_mask(dev->ctx, dev->user, image, rect, ctm);
}
...
{
	if (dev->begin_mask)
		dev->begin_mask(dev->ctx, dev->user, area, luminosity, colorspace, bc);
}
...
{
	if (dev->begin_group)
		dev->begin_group(dev->ctx, dev->user, area, isolated, knockout, blendmode, alpha);
}
...
{
	if (dev->end_group)
		dev->end_group(dev->ctx, dev->user);
}
...
{
	if (dev->begin_tile)
		dev->begin_tile(dev->ctx, dev->user, area, view, xstep, ystep, ctm);
}
