...
public:
		...
		pal8bit(NULL)
	{
		...
		{
			pix = fz_new_pixmap_with_limit(ctx, fz_device_bgr, pixmap->w, pixmap->h);
			if (!pix)
			{
...
public:
	...
	userData(fz_context *ctx, HDC hDC, fz_bbox clip) : stack(new userDataStackItem()), dev(NULL),
	...
	float getAlpha(float alpha=1.0) const { return (stack ? stack->alpha * alpha : alpha); }
...
static Brush *
gdiplus_get_brush(fz_context *ctx, void *user, fz_colorspace *colorspace, float *color, float alpha)
{
	float rgb[3];
	
	if (!((userData *)user)->t3color)
		fz_convert_color(((userData *)user)->ctx, colorspace, color, fz_device_rgb, rgb);
	else
...
gdiplus_get_font(userData *user, fz_font *font, float height, float *out_ascent)
{
	...
	if (!user->fontCollections)
		user->fontCollections = fz_new_hash_table(((userData *)user)->ctx, 13, sizeof(font->name));
	...
	if (!collection)
	{
		...
		fz_hash_insert(user->ctx, user->fontCollections, font->name, collection);
	}
...
fz_gdiplus_fill_path(fz_context *ctx, void *user, fz_path *path, int evenodd, fz_matrix ctm,
	fz_colorspace *colorspace, float *color, float alpha)
{
	GraphicsPath *gpath = gdiplus_get_path(path, ctm, evenodd);
	Brush *brush = gdiplus_get_brush(ctx, user, colorspace, color, alpha);
...
extern "C" static void
fz_gdiplus_stroke_path(fz_context *ctx, void *user, fz_path *path, fz_stroke_state *stroke, fz_matrix ctm,
	fz_colorspace *colorspace, float *color, float alpha)
{
...
extern "C" static void
fz_gdiplus_clip_path(fz_context *ctx, void *user, fz_path *path, fz_rect *rect, int evenodd, fz_matrix ctm)
{
	GraphicsPath *gpath = gdiplus_get_path(path, ctm, evenodd);
...
extern "C" static void
fz_gdiplus_clip_stroke_path(fz_context *ctx, void *user, fz_path *path, fz_rect *rect, fz_stroke_state *stroke, fz_matrix ctm)
{
	GraphicsPath *gpath = gdiplus_get_path(path, ctm);
...
gdiplus_render_text(userData *user, fz_text *text, fz_matrix ctm, Brush *brush, GraphicsPath *gpath=NULL)
{
	...
	if (!user->outlines)
		user->outlines = fz_new_hash_table(((userData *)user)->ctx, 509, sizeof(ftglyphkey));
	...
	{
		GraphicsPath *glyph = ftrenderglyph(((userData *)user)->ctx, text->font, text->items[i].gid, user->outlines);
		if (!glyph)
			continue;
	...
	fz_colorspace *colorspace, float *color, float alpha, GraphicsPath *gpath=NULL)
{
	...
	if (gpath)
		fz_warn(((userData *)user)->ctx, "stroking Type 3 glyphs is not supported");
...
extern "C" static void
fz_gdiplus_fill_text(fz_context *ctx, void *user, fz_text *text, fz_matrix ctm,
	fz_colorspace *colorspace, float *color, float alpha)
{
	Brush *brush = gdiplus_get_brush(((userData *)user)->ctx, user, colorspace, color, alpha);
...
extern "C" static void
fz_gdiplus_stroke_text(fz_context *ctx, void *user, fz_text *text, fz_stroke_state *stroke, fz_matrix ctm,
	fz_colorspace *colorspace, float *color, float alpha)
{
	GraphicsPath gpath;
	if (text->font->ft_face)
...
extern "C" static void
fz_gdiplus_clip_text(fz_context *ctx, void *user, fz_text *text, fz_matrix ctm, int accumulate)
{
	GraphicsPath gpath;
	float black[3] = { 0 };
	if (text->font->ft_face)
...
extern "C" static void
fz_gdiplus_clip_stroke_text(fz_context *ctx, void *user, fz_text *text, fz_stroke_state *stroke, fz_matrix ctm)
{
	GraphicsPath gpath;
	float black[3] = { 0 };
	if (text->font->ft_face)
...
extern "C" static void
fz_gdiplus_ignore_text(fz_context *ctx, void *user, fz_text *text, fz_matrix ctm)
{
}

extern "C" static void
fz_gdiplus_fill_shade(fz_context *ctx, void *user, fz_shade *shade, fz_matrix ctm, float alpha)
{
	Rect clipRect;
	...
	fz_pixmap *dest = fz_new_pixmap_with_rect(((userData *)user)->ctx, fz_device_rgb, bbox);
	fz_clear_pixmap(dest);
	
	if (shade->use_background)
	{
		float colorfv[4];
		fz_convert_color(((userData *)user)->ctx, shade->colorspace, shade->background, fz_device_rgb, colorfv);
		colorfv[3] = 1.0;
	...
	fz_paint_shade(((userData *)user)->ctx, shade, ctm, dest, bbox);
	
	ctm = fz_concat(fz_scale(dest->w, -dest->h), fz_translate(dest->x, dest->y + dest->h));
	((userData *)user)->drawPixmap(dest, ctm, alpha);
	
	fz_drop_pixmap(((userData *)user)->ctx, dest);
}

extern "C" static void
fz_gdiplus_fill_image(fz_context *ctx, void *user, fz_pixmap *image, fz_matrix ctm, float alpha)
{
	((userData *)user)->drawPixmap(image, ctm, alpha);
}

extern "C" static void
fz_gdiplus_fill_image_mask(fz_context *ctx, void *user, fz_pixmap *image, fz_matrix ctm,
	fz_colorspace *colorspace, float *color, float alpha)
{
	float rgb[3];
	if (!((userData *)user)->t3color)
	...
	fz_pixmap *img2 = fz_new_pixmap_with_limit(((userData *)user)->ctx, fz_device_rgb, image->w, image->h);
	if (!img2)
		return;
	...
	fz_drop_pixmap(((userData *)user)->ctx, img2);
}

extern "C" static void
fz_gdiplus_clip_image_mask(fz_context *ctx, void *user, fz_pixmap *image, fz_rect *rect, fz_matrix ctm)
{
	((userData *)user)->pushClipMask(image, ctm);
}

extern "C" static void
fz_gdiplus_pop_clip(fz_context *ctx, void *user)
{
	((userData *)user)->popClip();
}

extern "C" static void
fz_gdiplus_begin_mask(fz_context *ctx, void *user, fz_rect rect, int luminosity,
	fz_colorspace *colorspace, float *colorfv)
{
	float rgb[3] = { 0 };
	if (luminosity && colorspace && colorfv)
		fz_convert_color(((userData *)user)->ctx, colorspace, colorfv, fz_device_rgb, rgb);
	
	((userData *)user)->recordClipMask(rect, !!luminosity, rgb);
}

extern "C" static void
fz_gdiplus_end_mask(fz_context *ctx, void *user)
{
	((userData *)user)->applyClipMask();
}

extern "C" static void
fz_gdiplus_begin_group(fz_context *ctx, void *user, fz_rect rect, int isolated, int knockout,
...
extern "C" static void
fz_gdiplus_end_group(fz_context *ctx, void *user)
{
	((userData *)user)->popClip();
}

extern "C" static void
fz_gdiplus_begin_tile(fz_context *ctx, void *user, fz_rect area, fz_rect view, float xstep, float ystep, fz_matrix ctm)
{
	((userData *)user)->recordTile(view, area, ctm, xstep, ystep);
}

extern "C" static void
fz_gdiplus_end_tile(fz_context *ctx, void *user)
{
	((userData *)user)->applyTiling();
}

extern "C" static void
fz_gdiplus_free_user(fz_context *ctx, void *user)
{
